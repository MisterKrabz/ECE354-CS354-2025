...
int valid_sudoku_board(int **board, int size) {
	...
	for(int i = 0; i < size; i++){
		// temporary array to track numbers in each row 
		int *row_check = (int *)malloc((size+1)*sizeof(int));
		if(row_check == NULL){
			exit(1);
		}
	...
	// checking columns 
	for (int j = 0; j < size; j++){
		// temporary array to track numbers in each col
		int *col_check = (int *)malloc((size + 1)*sizeof(int));
		if(col_check == NULL){
			exit(1);
		}
		for(int k = 0; k <= size; k++){
