...
int free_block(void *ptr) {
	...
	if((void*)header < (void*)heap_start || (void*)header >= heap_end){
		return -1;
	}
	...
	int current_size = header->size_status & ~3;
    	int is_prev_alloc = header->size_status & 2;

	blockHeader *next_header = (blockHeader *)((char *)header + current_size);
	int is_next_free = (next_header->size_status != 1 && !(next_header->size_status & 1));

	// Previous and Next are both allocated
	if (is_prev_alloc && !is_next_free) {
        	header->size_status &= ~1; // Just free the current block
        	blockHeader *footer = (blockHeader *)((char *)header + current_size - sizeof(blockHeader));
        	footer->size_status = current_size;
        	if (next_header->size_status != 1) {
            		next_header->size_status &= ~2; // Update next block's p-bit
        	}
    	}

	// Previous is allocated, Next is free.
	else if (is_prev_alloc && is_next_free) {
        	int next_size = next_header->size_status & ~3;
        	int new_size = current_size + next_size;
        	header->size_status = new_size | 2; // Keep prev allocated p-bit
       	 	blockHeader *footer = (blockHeader *)((char *)header + new_size - sizeof(blockHeader));
	...
	// Previous is free, Next is allocated
        	...
        	int new_size = prev_size + current_size;
        	// New header is prev_header. Preserve its p-bit.
        	prev_header->size_status = new_size | (prev_header->size_status & 2);
        	blockHeader *footer = (blockHeader *)((char *)prev_header + new_size - sizeof(blockHeader));
        	footer->size_status = new_size;
        	if (next_header->size_status != 1) {
            		next_header->size_status &= ~2; // Update next block's p-bit
        	}
	...
	// Previous and Next are both free.
        	...
        	blockHeader *prev_header = (blockHeader *)((char *)header - prev_size);
        	int next_size = next_header->size_status & ~3;
        	int new_size = prev_size + current_size + next_size;
        	// New header is prev_header. Preserve its p-bit.
        	prev_header->size_status = new_size | (prev_header->size_status & 2);
        	blockHeader *footer = (blockHeader *)((char *)prev_header + new_size - sizeof(blockHeader));
        	footer->size_status = new_size;
